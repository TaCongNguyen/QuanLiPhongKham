...
namespace DAO
{
    public class KeToaThuoc_DAO
    {
        ...
        public static List<CTTT> LoadCTTT(int MaPK)
        {
            ...
            {
                CTTT DTO = new CTTT();
                DTO.MaPK1 = int.Parse(row["MaPK"].ToString());
                DTO.MaThuoc1 = int.Parse(row["MaThuoc"].ToString());
                DTO.SoLuong1 = int.Parse(row["SoLuong"].ToString());
                DTO.DonGia1 = float.Parse(row["DonGia"].ToString());
                DTO.CachDung1 = row["CachDung"].ToString();
            }
            return listbn;
        }
